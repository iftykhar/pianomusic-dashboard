...
interface DeleteContentConfirmModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  isLoading: boolean;
...
const DeleteExerciseContentConfirmModal: React.FC<
  DeleteContentConfirmModalProps
> = ({ isOpen, onClose, onConfirm, isLoading, title }) => {
  return (
        ...
        <div className="bg-linear-to-br from-rose-500 to-rose-600 p-8 text-white flex flex-col items-center text-center">
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-rose-100 font-medium">
              You are about to remove content{" "}
              <span className="font-bold underline">
                {title || "this item"}
              </span>
              . This can be undone if you have backups, but here it&apos;s
              permanent.
            </DialogDescription>
          </DialogHeader>
        </div>
...
export default DeleteExerciseContentConfirmModal;
